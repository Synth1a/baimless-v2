...
void animation_state::create_state(C_AnimState* state, C_BaseEntity* player) {
	...
	static auto fn = reinterpret_cast<Fn>(Utils::FindSignature("client.dll", "55 8B EC 56 8B F1 B9 ? ? ? ? C7 46"));
	fn(state, player);
}
...
void animation_state::reset_state(C_AnimState* state) {
	...
	static auto fn = reinterpret_cast<Fn>(Utils::FindSignature("client.dll", "56 6A 01 68 ? ? ? ? 8B F1"));
	fn(state);
}
...
{
	if (!state)
		UpdateAnimState = Utils::FindSignature("client.dll", "55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54 24");
	if (!UpdateAnimState)
		return;
