...
namespace interfaces
{
    ...
    void Init()
    {
		gameUI			= (CGameUI*)g_InterfaceUtils->grabInterface("client.dll", "GameUI011");
		//gameUI        = FindClass<CGameUI>("client.dll", "GameUI011");
		...
		g_pGlobalVars					= **(CGlobalVarsBase***)((*(uintptr_t**)g_pClientDll)[0] + 0x1F);
		...
		g_pIViewRenderBeams				= *reinterpret_cast< IViewRenderBeams ** >(Utils::FindSignature("client.dll", "8D 43 FC B9 ? ? ? ? 50 A1") + 4);
		...
		m_pGlowObjManager				= *(CGlowObjectManager**)(Utils::FindSignature("client.dll", "0F 11 05 ? ? ? ? 83 C8 01") + 3);
		g::g_iModelBoneCounter			= **reinterpret_cast<unsigned long**>(Utils::FindSignature("client.dll", "80 3D ? ? ? ? 00 74 16 A1") + 10); //80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81
		...
		g_Input							= *(CInput**)(Utils::FindSignature("client.dll", "B9 ? ? ? ? F3 0F 11 04 24 FF 50 10") + 1);
		g_D3DDevice9					= **(IDirect3DDevice9***)(Utils::FindSignature("shaderapidx9.dll", "A1 ? ? ? ? 50 8B 08 FF 51 0C") + 1);
		g_pGameRules					= *reinterpret_cast<CGameRules***>(Utils::FindSignature("client.dll", "A1 ? ? ? ? 85 C0 0F 84 ? ? ? ? 80 B8 ? ? ? ? ? 74 7A") + 1);
		...
		for (auto sex : fart) WriteProcessMemory(GetCurrentProcess(), (LPVOID)Utils::FindSignature(sex, "55 8B EC 56 8B F1 33 C0 57 8B 7D 08"), &amongus, 7, 0);
