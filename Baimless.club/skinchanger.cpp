...
void Fullupdate()
{
	static auto clear_hud_weapon_icon_fn =
		reinterpret_cast<std::int32_t(__thiscall*)(void*, std::int32_t)>(
			Utils::FindSignature("client.dll", "55 8B EC 51 53 56 8B 75 08 8B D9 57 6B FE 2C"));

	auto element = FindHudElement<std::uintptr_t*>("CCSGO_HudWeaponSelection");

	auto hud_weapons = reinterpret_cast<hud_weapons_t*>(std::uintptr_t(element) - 0x20);
...
static auto make_glove(int entry, int serial) -> C_BaseAttributableItem* {
	...
	assert(glove); {
		static auto set_abs_origin_addr = Utils::FindSignature(("client.dll"), "55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8");
